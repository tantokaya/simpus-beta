...
public function ItemAwalApotek($id){
	...
	function get_all_apotek_keluar() {
		...
		$query = $this->db->get();
		return $query->result_array();
		/*$this->db->select('brg_apotek_keluar_header.*, unit_farmasi.nama_unit_farmasi');
		$this->db->from('brg_apotek_keluar_header');
		$this->db->order_by('kd_keluar', 'desc');
		$this->db->join('unit_farmasi', 'brg_apotek_keluar_header.kd_unit_farmasi = unit_farmasi.kd_unit_farmasi');
		$query = $this->db->get();
		return $query->result_array();*/
...
/**********************************************************************************************************/
	...
	function get_list_pasien($rm)
	{
		...
		*/
		...
		$this->db->select('pasien.nm_lengkap,pasien.no_reg,pasien.nik,pasien.tempat_lahir,pasien.tanggal_lahir,jenis_kelamin.jenis_kelamin,pasien.no_kk,pasien.nm_kk,pasien.kd_rekam_medis,pasien.nm_ibu,pasien.kd_puskesmas,puskesmas.nm_puskesmas');
		$this->db->from('pasien');
		//$this->db->join('golongan_darah', 'pasien.kd_gol_darah = golongan_darah.kd_gol_darah');
		$this->db->join('jenis_kelamin', 'pasien.kd_jenis_kelamin = jenis_kelamin.kd_jenis_kelamin');
		//$this->db->join('cara_bayar', 'pasien.kd_bayar = cara_bayar.kd_bayar');
	...
	function get_pasien_rekam_medis($rm)
	{
		$this->db->select('*');
	...
	{
		$this->db->select('bed.*, kamar.no_kamar, kamar.kd_ruangan, ruangan.nm_ruangan');
		$this->db->from('bed');
		$this->db->join('kamar', 'bed.kd_kamar = kamar.kd_kamar');
		$this->db->join('ruangan', 'kamar.kd_ruangan = ruangan.kd_ruangan');
		$this->db->where('bed.kd_bed', $kd_bed);
